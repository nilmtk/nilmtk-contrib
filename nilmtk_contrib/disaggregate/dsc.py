...
class DSC(Disaggregator):
    ...
    def learn_dictionary(self, appliance_main, app_name):
        ...
        print("RMSE reconstruction for appliance %s is %s" % (app_name, mean_squared_error(reconstruction, appliance_main) ** (0.5)))
    ...
    def discriminative_training(self, concatenated_activations, concatenated_bases, verbose=100):
        ...
        for i in range(self.iterations):
            ...
            model = SparseCoder(
                dictionary=predicted_b.T, positive_code=True, transform_algorithm='lasso_lars', transform_alpha=self.sparsity_coef
            )
    ...
    def print_appliance_wise_errors(self, activations, bases):
        ...
        for cnt, i in enumerate(self.power):
            ...
            print("Error for ", i, " is ", mean_squared_error(pred, X) ** (0.5))
    ...
    def partial_fit(self, train_main, train_appliances, **load_kwargs):
        ...
        if len(train_main) > 10:
            ...
            print("--" * 15)
            model = SparseCoder(
                dictionary=concatenated_bases.T, positive_code=True, transform_algorithm='lasso_lars', transform_alpha=self.sparsity_coef
            )
            ...
            model = SparseCoder(
                dictionary=optimal_b.T, positive_code=True, transform_algorithm='lasso_lars', transform_alpha=self.sparsity_coef
            )
        ...
        for test_main in test_main_list:
            ...
            for cnt, app_name in enumerate(self.power):
                ...
                predicted_usage = np.matmul(
                    self.reconstruction_bases[:, start_comp : start_comp + n_comps],
                    predicted_activations[start_comp : start_comp + n_comps, :],
                )
                start_comp += n_comps
