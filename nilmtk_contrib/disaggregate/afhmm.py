    ...
    def disaggregate_thread(self, test_mains, index, d):
        ...
        for epoch in range(6):
            ...
            if epoch % 2 == 1:
                ...
                if flag == 0:
                    ...
                    for appliance_id in range(self.num_appliances):
                        ...
                        for t in range(len(test_mains)):
                            matrix = cvx.Variable(
                                shape=(self.default_num_states, self.default_num_states), name='variable_matrix-%s-%d' % (appliance_id, t)
                            )
                ...
                for t in range(len(test_mains)):
                    term_4 += 0.5 * ((test_mains[t][0] - total_observed_reading[t][0]) ** 2 / (sigma[t] ** 2))
                    term_3 += 0.5 * (np.log(sigma[t] ** 2))
